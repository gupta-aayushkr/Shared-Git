...
for container in containers:
    ...
    # Extract media link and type
    media_link, media_type = get_media_info(container)
    
    # Extract and format post date
    post_date = get_text(container, "div", {"class": "ml4 mt2 text-body-xsmall t-black--light"})
    post_date = get_actual_date(post_date)
    
    # Reactions (likes)
    reactions_element = container.find_all(lambda tag: tag.name == 'button' and 'aria-label' in tag.attrs and 'reactions' in tag['aria-label'].lower())
    
    # Check if reactions_element contains the button with the reaction count
    if reactions_element:
        # Look for the span within the button that contains the count
        reaction_count_span = reactions_element[0].find("span", class_="social-details-social-counts__reactions-count")
        post_reactions = reaction_count_span.text.strip() if reaction_count_span else 0
    else:
        post_reactions = 0  # Default if reactions_element is not found
    
    print(f"Post Reactions = {post_reactions}")
    ...
    if comment_element:
        post_comments = comment_element[0].text.strip()
    else:
        post_comments = 0
    ...
    if shares_element:
        post_shares = shares_element[0].text.strip()
    else:
        post_shares = 0
    
    print(f"Post Shares = {post_shares}")
    
    # Store the extracted data in a dictionary and append to the posts_data list
    post_data = {
        "Post Text": post_text,
...
try:
    ...
    for col in ["Likes", "Comments", "Shares"]:
        df[col] = df[col].replace('', 0)  # Replace empty strings with 0
        df[col] = df[col].astype(int)  # Convert to integer

    # Sort by the number of likes (reactions)
